...
class TaskRepositoryTests(IsolatedAsyncioTestCase):

    async def test_get_by_id_should_return_none(self):
        single_query_result: Task | None = await Promise.resolve(partial(
    ...
    async def test_get_all_should_return_pageable(self):
        query_result: Pageable = await Promise.resolve(partial(TaskRepository.get_all, 0, 10))
